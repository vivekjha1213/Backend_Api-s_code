...
<head>
  ...
  <title>BLS_sweden_INHOUSE_MOVIPLUS</title>
  <meta charset="utf-8">
...
<body>
<div class="container">
  <h2>REPORT(BLS_sweden_INHOUSE_MOVIPLUS conversion tracker)</h2>
  <p>BLS_sweden_INHOUSE_MOVIPLUS</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/BLS_sweden_INHOUSE_MOVIPLUS?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From in_app_pin_verify where DATE(date) between '$today_date' and '$today_date' and serviceId='ARSH0126' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `in_app_pin_verify`  where DATE(date) between '$today_date' and '$today_date' and serviceId='ARSH0126'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM in_app_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date' and serviceId='ARSH0126'";
...
<br>
<?php
$sql1="SELECT * From in_app_pin_verify where date(date) between '$today_date' and '$today_date' and serviceId='ARSH0126' order by id desc";
//exit();
//exit();
