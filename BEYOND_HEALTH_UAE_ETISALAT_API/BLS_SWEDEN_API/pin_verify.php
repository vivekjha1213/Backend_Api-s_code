...
if ($status == 0) {
  ...
  $results = file_get_contents('http://beyondhealth.info/Services/Filter/chk.php?id=361&op=Beyond_Life_Style_Olimob_Inhouse_Sweden');
  if ($results == 0) //MEANS PASS
  {
    $panda = "PASSED";
    $sql_subscription = "INSERT INTO  `in_app_pin_verify` (`serviceId`,`advName`,`pubName`,`serviceName`,`country`,`msisdn`,`otp`,`result`,`status`,`finalStatus`,`serviceDate`,`date`)  VALUES('$serviceId','$advName','$pubName','$serviceName','$country','$MSISDN','$pin','$result','$panda','$status','$serviceDate','$time_india_one')";
