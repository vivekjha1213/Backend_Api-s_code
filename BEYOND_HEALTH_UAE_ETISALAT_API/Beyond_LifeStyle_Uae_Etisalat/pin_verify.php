...
$sql = "SELECT `finalStatus` from `in_app_pin_request` WHERE `msisdn`='" .$msisdn. "' and 'serviceId`='ARSH0100' order by id desc limit 1";
...
function start_first($msisdn, $otp, $clickid, $token)
{
    ...
    if ($response == 'success') {
        $results = file_get_contents('http://beyondhealth.info/Services/Filter/chk.php?id=341&op=Beyond_Life_Style_Etisalat_Uae_Adbird_Inhouse');
