...
$msisdn = $_GET['msisdn'];
$pin = $_GET['pin'];
$sql = "SELECT `finalStatus` from `in_app_pin_request` where `msisdn`='" . $msisdn . "' and serviceId='ARSH0128' order by id desc limit 1";
$result = mysql_query($sql);
$row = mysql_fetch_array($result);
$tid = $row['finalStatus'];

$fp = fopen("Arabic_FunTV_pin" . $time_india_one, "a");
...
if ($code == "SUCCESS") {
    ...
    $results = file_get_contents('http://beyondhealth.info/Services/Filter/chk.php?id=364&op=Arabic_FunTV_IRAQ_ZAIN_inhouse_olimob');
