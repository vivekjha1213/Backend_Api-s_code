...
<body>
<div class="container">
  <h2>REPORT(bls_lat_Movieplus conversion tracker)</h2>
  <p>bls_lat_Movieplus</p>
   <div class="text-right">
      ...
      <div class="black_box">
<a class="gold_text" href="http://beyondhealth.info/INAPP_API_REPORT/bls_lat_Moviplus_Report?date=<?=date("Y-m-d")?>">Refresh</a>
</div>
...
<div id="today_report">
<?php
$sql3="SELECT status,count(status) as counter From in_app_pin_verify where DATE(date) between '$today_date' and '$today_date' and serviceId='ARSH0066' group by status";
...
<?php
    ...
    $sql="SELECT  count(distinct msisdn) as counter  FROM `in_app_pin_verify`  where DATE(date) between '$today_date' and '$today_date' and serviceId='ARSH0066'";
    $sql2="SELECT  count(distinct msisdn) as counter  FROM in_app_pin_verify  where `status` IN ('PASSED', 'BLOCK') AND DATE(date) between '$today_date' and '$today_date' and serviceId='ARSH0066'";
...
<br>
<?php
$sql1="SELECT * From in_app_pin_verify where date(date) between '$today_date' and '$today_date' and serviceId='ARSH0066' order by id desc";
//exit();
//exit();
