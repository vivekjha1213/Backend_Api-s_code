...
if($service=="Bavyesh-EG-Funzstation")
{
                  ...
                  // $TransactionId=rand();
									...
									$fp = fopen('Egypt_pin_request_log_'.date("Y-m-d"), 'a');
                  ...
                  $sql = "INSERT INTO `in_app_pin_request` (`serviceId`,`advName`,`pubName`,`serviceName`,`country`,`msisdn`,`result`,`status`,`finalStatus`,`serviceDate`,`date`)  VALUES('$serviceId','$advName','$pubName','$serviceName','$country','$MSISDN','$result','$operator','$final_response','$serviceDate','$time_india_one')";
                  ...
                  mysql_query($sql);
							        ...
							        {
							            echo json_encode(array('success' => 1,'contractID' =>$contractID));
							            exit();
							        }
							        elseif($final_response=="This user already subscribed to the given product")
							        {
							            echo json_encode(array('success' => "This user already subscribed to the given product"));
							        }
							        elseif($final_response=="Please enter valid phone number for the selected mobile operator.")
							        {
							            echo json_encode(array('success' => "Please enter valid phone number for the selected mobile operator."));
							        }
							        elseif($final_response=="MSISDN Can't be null")
							        {
							            echo json_encode(array('success' => "MSISDN Can't be null"));
							        }
							        elseif($final_response=="This mobile number is blacklisted")
							        {
							            echo json_encode(array('success' => "This mobile number is blacklisted"));
							        }
							        else
							        {
