<?php
// $service = file_get_contents('http://beyondhealth.info/Services/Switch/api.php?id=56');
// if ($service == "ENYOGLOBAL_Funzstation_EGYPT") {
	include("connect.php");
	$pin = $_GET['pin'];
	$msisdn = $_GET['msisdn'];
	$cid = "N.A";
	...
	function verify_otp($pin, $msisdn, $cid, $contractID, $operator, $language)
	{
		...
		$fp = fopen('ENYOGLOBAL_Funzstation_EGYPT_PIN_VERIFY_LOG' . date("Y-m-d"), 'a');
		...
		if ($final_response == 'null' || $final_response == "") {
			...
			$ans = file_get_contents('http://beyondhealth.info/Services/Filter/chk.php?id=271&op=ENYOGLOBAL_Funzstation_EGYPT');
			if ($ans == 0) {
				$status = "PASSED";
