...
function launcher($msisdn,$pin,$clickid) {
	...
	if ($otp != '') {
		...
		//$sql_subscription = "INSERT INTO `saudi_arabia_timwe_pin_verify`(`cid`, `msisdn`, `subscriptionResult`, `message`,`date_india`,`date_saudi_arabia`) VALUES ('$clickid', '$msisdn', '$subscriptionResult', '$message','$time_india','$date_saudi_arabia')";
...
if($service=="MOBILY_INFOCELL")
{
   ...
   // $sql="SELECT `data` from `game2play_mobily_infocell_pin_request` WHERE msisdn='".$msisdn."' order by id desc limit 1";
    ...
    $fp = fopen('Mobily_infocell_Game2play_pin_verify_log'.date("Y-m-d"), 'a');
    ...
    // Submit the POST request
      ...
      $reason=$check['result']['response']; 
       ...
       if ($code=="1") {
         ...
         $results=file_get_contents('http://beyondhealth.info/Services/Filter/chk.php?id=83&op=Mobily_infocell_game2play_pro');
          if($results==0)//MEANS PASS
                        {


                            $panda="PASSED";
                                   $sql="INSERT INTO  `game2play_mobily_infocell_pin_verify` (`cid`,`msisdn`,`pin`,`subscriptionResult`,`status`,`date`) VALUES('".$clickid."','".$msisdn."','".$pin."','".$result."','".$panda."','".$time_india."')";
                                 mysql_query($sql);
             ...
             $sql="INSERT INTO  `game2play_mobily_infocell_pin_verify` (`cid`,`msisdn`,`pin`,`subscriptionResult`,`status`,`date`) VALUES('".$clickid."','".$msisdn."','".$pin."','".$result."','".$reason."','".$time_india."')";
        mysql_query($sql);
