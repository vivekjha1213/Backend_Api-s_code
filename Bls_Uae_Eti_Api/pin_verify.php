...
$sql = "SELECT `finalStatus` from `in_app_pin_request` WHERE `msisdn`='" . $msisdn . "' and 'serviceId`='ARSH0109' order by id desc limit 1";
...
function start_first($msisdn, $otp, $clickid, $token)
{
    ...
    if ($response == 'success') {
        ...
        mysql_query($sql);
        echo json_encode(array('response' => 'SUCCESS', 'errorMessage' => 'Success'));
        exit();
    } else {
        $status = "$response";
        $sql = "INSERT INTO  `in_app_pin_verify` (`serviceId`,`advName`,`pubName`,`serviceName`,`country`,`msisdn`,`otp`,`result`,`status`,`finalStatus`,`serviceDate`,`date`)  VALUES('$serviceId','$advName','$pubName','$serviceName','$country','$msisdn','$otp','$result','$response','$status','$serviceDate','$time_india_one')";
        mysql_query($sql);
