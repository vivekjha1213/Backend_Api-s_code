...
{
    $results =file_get_contents('http://beyondhealth.info/Services/Filter/chk.php?id=288&op=MOJO_DIGIFISH_BEYONDHEALTH_UAE');
 ...
 // $sql_subscription = "INSERT INTO `beyondlifestyle_uae_alt_pin_verify`(`cid`,`msisdn`,`response`, `status`,`date_india`,`otp`) VALUES ('$clickid', '$msisdn','$response','$status','$date_india','$otp')";
      ...
      $sql = "INSERT INTO  `in_app_pin_verify` (`serviceId`,`advName`,`pubName`,`serviceName`,`country`,`msisdn`,`otp`,`result`,`status`,`finalStatus`,`serviceDate`,`date`)  VALUES('$serviceId','$advName','$pubName','$serviceName','$country','$msisdn','$otp','$result','$panda','$response','$serviceDate','$time_india_one')";
      mysql_query($sql);
  
  
      echo json_encode(array('errorMessage' => 'Pin Verified', 'response' => 'SUCCESS'));
  ...
  
    {
      ...
      echo json_encode(array('errorMessage' => 'Failure Message String', 'response' => 'FAIL'));
      exit();
    }
    ...
    $status="Failed";
    $sql = "INSERT INTO  `in_app_pin_verify` (`serviceId`,`advName`,`pubName`,`serviceName`,`country`,`msisdn`,`otp`,`result`,`status`,`finalStatus`,`serviceDate`,`date`)  VALUES('$serviceId','$advName','$pubName','$serviceName','$country','$msisdn','$otp','$api_url','$response','$status','$serviceDate','$time_india_one')";
    mysql_query($sql);
