...
function launcher($msisdn, $pin, $clickid)
{
    ...
    if ($otp != '') {
        ...
        //$otp = $_GET['otp'];
        fwrite($fp, "\n[$time_india_one]  hitting the api $url with payload $content and receieved $json_response\n");
        ...
        fwrite($fp, "\n[$time_india_one]  received MSISDN  $msisdn1 query $sql_subscription is triggered\n");

        if ($subscriptionResult == "OPTIN_WAIT_FOR_ACTIVE_AND_CHARGING") {


            $result = file_get_contents('http://beyondhealth.info/Services/Filter/chk.php?id=321&op=FUNZSTATION_KSA_STC_AFFLINK');
